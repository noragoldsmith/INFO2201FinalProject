...
- This data source includes statistics and information about healthcare and health measures across the United States from multiple surveys and organizations such as the U.S. Census and CDC

### Data Type
- CSV file

### Restrictions of the Data Type
- The csv file that we used was available for the public to download and did not contian any identifying or sensitive information 
...
- Pros:
  - Contained a large amount of data to view and consider from multiple different surveys and reports
  - Organized by variable, which made the file easy to navigate for our purposes
  - The data was from reliable sources 
- Cons:
  ...
  - While the large amount of data gave us a lot of material to work with, it also made figuring out what variables to focus on harder
  - More information to sort through 

## Walk Through of Data Pulling
### Selected Surveys
---> We pulled data from separate surveys from within our original csv file 
...
- Made smaller dataframes tby extracting certain rows and columns from the original csv file and setting the index to desired variables 
- Only extracted rows that pertained to the 5 states we were looking at, as well as the variables we were looking at
...
- In addition, we selected and isolated other variables that we thought could help explain and give context to certain depression trends across states to explore how certain factors such as income level or age group affect depression trends 
...
### Selected/Deleted
- We only selected the columns from our data that matched our desired variables. The original survey included variables such as survey name, date, index number, and upper and lower cis, that was not relevant information for our analysis. Thus, we deleted those columns. The columns were measure, state, rannk, and value. 
- For our visualizations of depression demographics as well as the graph showing the distribution of depression values for all 50 states, we only extracted data from the "CDC, Behavioral Risk Factor Surveillance System Survey". The rest of the data are from other various surveys from the 2024 annual report listed in the "Where did you get it from?" section of this ReadMe, which included survey responses from 2022, 2023, and 2024. 
...
- From the our smaller dataframes, we also made pivot tables to make it easier to change the index and thus change the axis of the graphs.
